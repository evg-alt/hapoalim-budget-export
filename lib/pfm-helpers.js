...
async function switchBudgetMode(page, mode = 'income', { timeoutMs = 30_000 } = {}) {
  ...
  await frame.getByRole('heading', { name: headingPattern }).waitFor({ state: 'visible', timeout: timeoutMs });
  await frame.getByText(totalLabel, { exact: true }).waitFor({ state: 'visible', timeout: timeoutMs }).catch(() => null);
