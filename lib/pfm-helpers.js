...
async function hasBudgetTableReady(frame) {
  const [openAll, categoryColumn] = await Promise.all([
    frame.getByText(/לפתוח הכל/).first()
...
async function switchBudgetMode(page, mode = 'income', { timeoutMs = 12_000 } = {}) {
  let frame = await getPFMFrame(page);
  if (await isMonthEmpty(frame)) return frame;

  // Already on the requested view — do not re-click (can race SPA).
  if (await hasViewHeading(frame, mode) && await hasBudgetTableReady(frame)) {
    return frame;
  }
  ...
  await button.click({ timeout: 5000 }).catch(() => button.click({ force: true }));
  await sleep(400);
  ...
  while (Date.now() < deadline) {
    ...
    // Heading OR category table is enough — July after June can lag the heading.
    if (await hasViewHeading(activeFrame, mode) || await hasBudgetTableReady(activeFrame)) {
      return activeFrame;
    }
    await sleep(200);
  }
  ...
  if (await hasBudgetTableReady(finalFrame)) return finalFrame;
