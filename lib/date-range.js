...
function parseCollectRange(input) {
  ...
  let startDay;
  if (startIsMonthOnly || startPart.day === null) {
    startDay = 1;
  } else {
  ...
  let endDay;
  if (endIsMonthOnly || endPart.day === null) {
...
function monthTabsForRange(range, availableTabs) {
  ...
  if (tabs.length === 0) {
    if (!availableTabs.length) {
      throw new Error(
        `No month tabs found on the budget page for range ${range.start}–${range.end}.`,
      );
    }

    throw new Error(
      `No bank month tabs overlap range ${range.start}–${range.end}. ` +
        `Available on page: ${availableTabs.join(', ')}`,
    );
  }
